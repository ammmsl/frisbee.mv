...
import { kickoffToInput, toMVTIso } from '@/lib/league-schedule'
...
function EditFixtureForm({
  ...
  async function handleSave() {
    ...
    setSaving(true)
    setError('')
    const res = await fetch(`/api/league/admin/fixtures/${fixture.match_id}`, {
      method: 'PATCH',
...
export function FixtureList({
  ...
  async function handleDelete(matchId: string) {
    setDeleting(matchId)
    const res = await fetch(`/api/league/admin/fixtures/${matchId}`, { method: 'DELETE' })
    setDeleting(null)
