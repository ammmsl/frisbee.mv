...
**Last updated:** 2026-07-13 (Waves 4+5 complete — data section + per-page macros)
...
- [x] Live Stats Bar — 3 StatTile components; since Wave 1 (2026-07-12) values are a live server-side fetch from PivotAttendance via `lib/pickup-sheets.ts` (cached 300 s, labelled denominators, bar hides on fetch failure) — no hardcoded counts
- [x] Next Session block — `lib/session.ts` helper; pure server-side MVT arithmetic (UTC+5 offset, no external libs); correct Tue/Fri logic with 20:00 cutoff; "Get Directions" ghost link → Google Maps
...
| 2026-07-12 | **Wave 1 (correctness & security) complete** on branch `wave-1-correctness` — see `docs/wave-1-report.md`. Audit plans 001 (league cache invalidation + key collisions), 003 (admin proxy bypass + handler-level auth + secret fail-fast), 004 (pickup payments server-side, credentials out of the bundle) all DONE. Hallmark Phase 1 bug items applied (#22 deviates — see report); home stats now a live labelled fetch. All 18 spec §6 amendments + T03 wording calls applied (Timeline rebuilt, 8:00 PM everywhere, fees 250/350, atoll "event & outreach activity"). T04 closed (news schema has no category field; minimal spec in wayfinder/assets). `npx tsc --noEmit` + `npm run build` clean after every package. Manual steps: see report §Leftovers. |
| 2026-07-12 | **Waves 2 + 3 complete** on branch `wave-2-3-structure-and-chrome` — see `docs/wave-2-3-report.md`. Wave 2: audit plan 002 DONE — all 30 cached league loaders centralized in `lib/league-queries.ts` (shared LEAGUE_CACHE, keys preserved, canonical `getActiveSeason` under new `-v2` key); public league pages no longer own SQL; dead `getConsecutiveWeeks()` deleted. Wave 3: Hallmark Phase 2 chrome — Fraunces display serif (`font-display`, hero-h1/major-h2 only), tinted pacific paper (oklch), SiteNav rewritten as static N6 newspaper masthead (hero-sentinel + scroll choreography deleted, layouts drop pt-16), SiteFooter rewritten as Ft5 statement, league bridge back-link, lucide-react dropped (11 icons hand-rolled in `app/league/_components/icons.tsx`), home hero now content-height left-biased (transitional until Phase 3a). `--text-muted` darkened to #626d7b for AA on tinted surface. `npx tsc --noEmit` + `npm run build` clean after each package. |
| 2026-07-13 | **Waves 4 + 5 complete** on branch `wave-4-5-data-and-macros` (**pushed to origin at owner request** for a Vercel preview) — see `docs/frisbee-mv-wave-4-5-report.md`. Wave 4: T04 implemented (CHECK-constrained `news_posts.category`, default-safe pre-migration — **owner must run `migrations/001-news-category.sql`**; NewsFilter rebuilt on the real field; Research badge); T05 prototype (`/data` Curated Index, defaults pending owner reaction); T06 transfer (36 report pages in `public/data/` + lib assets; agm-finance/nq-5 redacted; **oms-15 + b-tb1 withheld** on privacy-sweep role-tied-load findings — owner call). Wave 5: Hallmark Phase 3 — all seven per-page macros shipped one-commit-per-page (Marquee Hero, Long Document, Stat-Led, Workbench, Letter, Quote-Led, Typographic List), no display serif (owner call honored), stale orange hovers fixed on /play, `--note-special` orphan deleted. Followups: ADRs 0001/0002, CONTEXT.md, `.hallmark/log.json`, CLAUDE.md design section. `npx tsc --noEmit` + `npm run build` clean after every package and page. |
