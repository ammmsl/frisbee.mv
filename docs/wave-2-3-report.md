...
- **2a Fraunces — shipped, then REVERTED by owner call** (morning review: "the home font change is unnecessary — stick to the design tokens"). The font loading, `--font-display` token, discipline comment, and the three `font-display` usages on the home page were all removed; the site is single-font Inter again. Phase 3 macros must not reintroduce a display serif.
