...
import { makeMVTKickoff } from '@/lib/league-schedule'
import {
  type Season,
  type Fixture,
  type HolidayRange,
...
export default function FixturesPage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [fr, sr] = await Promise.all([
        fetch('/api/league/admin/fixtures'),
        fetch('/api/league/admin/season'),
      ])
      if (!fr.ok || !sr.ok) {
      ...
      // Load holidays
      const hrRes = await fetch(`/api/league/admin/holidays?seasonId=${seasonData.season_id}`)
      if (hrRes.ok) {
  ...
  async function handleConfirm() {
    ...
    const res = await fetch('/api/league/admin/fixtures/bulk', {
      method: 'PATCH',
