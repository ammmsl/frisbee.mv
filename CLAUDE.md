...
frisbee.mv is the website for the **Ultimate Frisbee Association (UFA)** — a registered sports association and provisional WFDF member. (Note: the name "Maldives Flying Disc Federation / MFDF" is used in WFDF registration contexts — a deliberate all-disc-sports mandate design, not merely a trademark workaround; all local branding uses UFA.) The site serves two audiences: the local player community (172 on the current roster; ~322 have ever played) and external parties (WFDF, sponsors, media, newcomers).
